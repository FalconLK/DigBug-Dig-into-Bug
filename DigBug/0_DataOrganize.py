import sys, gzip, os, csv, time, datetime
sys.path.append('.')
sys.path.append('..')
import pickle as p
from os.path import join, isfile
from GitSearch.MyUtils import write_file_a, read_file
...
def getTrainingReportsFromPickle(intersection_projects):
    ...
    if os.path.isfile('target_reports_train.txt'):
        os.remove('target_reports_train.txt')
...
def getBenchReportList():
    bench_reports= list()
    bench_base = '/extdsk/bug_localization/input_answer/input_bench4bl/'
...
def tableBuilder(final_training_list, localizable_list, unlocalizable_list, result_file, project):
    ...
    if (isfile(join(issue_pickle_path))):
        ...
        for idx, bid in enumerate(df.bid.values):
            ...
            if not bid in final_training_list:
                continue
            target_cnt += 1
            print(target_cnt)
...
if __name__ == "__main__":
    ...
    print(set(bench_project_list) & set(ifix_project_list))
