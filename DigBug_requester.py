...
from GitSearch.MyUtils import read_file, mkdir_p, cleanMetaPath, specialCharTokenizer
...
def generateResults(target, result_path, project, existing_file_list, input_path, answer_path, pp_option, cn_option):
    recommended_path = result_path + '%s_details/%s/%s/' % (project, pp_option, cn_option)

    final_path = result_path + '%s/' % project
    if not os.path.isdir(final_path):
        mkdir_p(final_path)

    result_file = final_path + '%s_%s_%s.csv' % (project, pp_option, cn_option)
    ...
    for path, dirs, files in os.walk(recommended_path):  # 디렉토리 밑 모든 파일들
        for file in files:
            ...
            #             candidate = token_java
            #
            #             result_list.insert(ranking_arrange, candidate)
            #             ranking_arrange += 1
            #             if candidate in answer_list:
            #                 write_file_a('success_candidates.txt', bug_id + ' : ' + candidate)
            #                 # csv_input_line.append(localize_floats([group, target, 'tmp', bug_id, version, answer_file_cnt, candidate, 1, 1, 1, 1, 1, 1, 1, ' ', candidate]))
            ...
            if flag == 1 and answer_file_cnt > file_found_cnt:
                for _ in range(answer_file_cnt - file_found_cnt):
                    csv_input_line.append([group, target, '', bug_id, version, answer_file_cnt, 'None', '0', '0', '0', '0', '0', '0'])
...
def expBench4BLForAll():
    ...
    # pp_option_list = ['SPC_STM']
    # pp_option_list = ['SPC_CMC']

    cn_option_list = ['OFF', 'ON']
    # cn_option_list = ['OFF']

    # version_list = ['original', 'oracle', 'predicted', 'augmented']
    # version_list = ['oracle']
    # version_list = ['augmented']
    ...
    for idx, target in enumerate(target_list):
        ...
        result_base_path = data_path + 'results_%s/' % (target.split('/')[1].lower())
        if not os.path.isdir(result_base_path):
            mkdir_p(result_base_path)
        else:
            cleanMetaPath(result_base_path)
        ...
        for idx, pp_option in enumerate(pp_option_list):
            ...
            for idx, cn_option in enumerate(cn_option_list):
                ...
                input_path = data_path + u'input_answer/input_bench4bl/%s/' % (target.split('/')[1])
                answer_path = data_path + u'input_answer/answer_bench4bl/%s/' % target.split('/')[1]

                project = str(target.split('/')[1].lower())  # hiv
                '''Result path'''
                result_path = result_base_path + u'%s_details/%s/%s/' % (project, pp_option, cn_option)
                fail_path = result_base_path + u'fail_' + u'%s_%s_%s.txt' % (project, pp_option, cn_option)

                # feature_flag = check_feature(result_path)   # if feature flag is True (description), the request should be POST method
                feature_flag = True
                ...
                '''현재 결과에 10656개 중에, 결과가 9110개, 9400개 이렇게 나오는데, 이건 각 파일의 이름은 같지만, 경로는 다르다. 즉, 이름이 같으면 중복제거에서 걸러지는거지.. 어쨋든 랭킹을 하기때문에.. 큰 차이는 없을거..'''

                existing_file_list = java_files_from_dir(project_path)
                generateResults(target, result_base_path, project, existing_file_list, input_path, answer_path, pp_option, cn_option)
