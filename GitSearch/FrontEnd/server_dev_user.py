...
def index(name=None):
    if request.method == 'POST':
        ...
        flag = 0    # 이건 실제 서버 1번인지 아닌지 확인. 0이면 테스트용임.
        ...
        if FLAG_BASELINE is True:
            PROJECT_INDEX_TO_USE = '%s_%s_baseline_latest_keyword' % (dataset, project.lower())
        else:
            PROJECT_INDEX_TO_USE = '%s_%s_latest_keyword' % (dataset, project.lower())
        ...
        try:
            if query:
                ...
                # ----------------------------------------------------------------------------------------------------
                so_result = query_index_without_html(query, result_file, REPORT_INDEX_TO_USE, PROJECT_INDEX_TO_USE, flag_bug_or_issue, group_project, project, dataset, dev_or_user, flag)
                print '\n' *20
...
def query_index_without_html(query, result_file, REPORT_INDEX_TO_USE, PROJECT_INDEX_TO_USE, flag_bug_or_issue, group_project, project, dataset, dev_or_user, flag):
    # GOOGLE
    ...
    yandex_jira = YandexJiraSearcher("%s%s" % (INDEX_BASE_PATH, REPORT_INDEX_TO_USE), flag_bug_or_issue, group_project, project, '%s_split' % dev_or_user, flag)# dataset)
