...
def generate_indices_from_projects(src, writer, counter, pp_option):
    ...
    for javafile in javafiles:
        ...
        try:
            ...
            if buildDocWithoutStructure(javafile, document, file_content, pp_option):
...
def buildDocWithoutStructure(javafile, document, file_content, pp_option):
    # print (file_content)
    # import time
    # time.sleep(100000)
    print javafile

    file_content = preProcessing_hunk(file_content)
    ...
    # print (file_content)
    # print ('---'*100)
    document.add(Field("code", file_content, Field.Store.YES, Field.Index.ANALYZED))
    return True

def indexingMain(src, dst, pp_option):
    import time
    ...
    try:
        ...
        counter = Counter()
        generate_indices_from_projects(src, writer, counter, pp_option)
        writer.close()
...
def preProcessing_hunk(file_content):
    file_content = preBasic(file_content)
    file_content = SPC(file_content)
    tokens = getTokens(file_content)
    tokens = CMC(tokens)
...
if __name__ == '__main__':
    ...
    src = '/extdsk/QT4BL_AWS/data_new_/hbase_5/code/original/'
    ...
    for idx, target in enumerate(target_list):
        for idx, pp_option in enumerate(pp_option_list):
            ...
            dst = '/home/ubuntu/Desktop/CoCaBu/GitSearch/Indices/%s_%s' % (target_project, pp_option)
            ...
            print(what_to_index)
            start_time = time.time()
            indexingMain(src, dst, pp_option)
            print('--- %s seconds ---' % (time.time() - start_time))