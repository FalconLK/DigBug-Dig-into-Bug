...
def generate_indices_from_projects(src, writer, counter, pp_option):
    ...
    for javafile in javafiles:
        ...
        try:
            ...
            if buildDocWithoutStructure(document, file_content, pp_option):
...
def buildDocWithoutStructure(document, file_content, pp_option):
    # print (file_content)
    # import time
    # time.sleep(100000)
...
if __name__ == '__main__':
    ...
    for idx, target in enumerate(target_list):
        print(''' %s iFixR Index ''' % target)

        for idx, pp_option in enumerate(pp_option_list):
            print(str(idx) + '/' + str(len(target_list)) + ' ////// ' + str(target))

            src = '/extdsk/iFixR/data/gitrepo_old/%s' % target
            dst = '/home/ubuntu/Desktop/CoCaBu/GitSearch/Indices/ifixr_%s_%s' % (target, pp_option)
