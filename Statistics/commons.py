...
def shellCallTemplate(cmd, enc='latin-1'):
    try:
...
def shellGitCheckout(cmd, enc='latin-1'):
    try:
        with Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True, encoding=enc) as p:
            output, errors = p.communicate()
            # print(output)
...
def get_venn_sections(sets):
    """
    Given tmp list of sets, return tmp new list of sets with all the possible
    mutually exclusive overlapping combinations of those sets.  Another way
    to think of this is the mutually exclusive sections of tmp venn diagram
